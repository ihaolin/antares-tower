...
let AppEdit = React.createClass({
  ...
  render(){
    ...
    // app name tip
    const nameInputTip = I18n.getText('input') + ' ' + I18n.getText('apps.name') + " " + I18n.getText('name.tip');
    const nameDisable = !(currentApp.id === null || currentApp.id === undefined);

    // app key tip
    const keyInputTip = I18n.getText('input') + ' ' + I18n.getText('apps.key');

     // app desc tip
    const descInputTip = I18n.getText('input') + ' ' + I18n.getText('apps.desc');

    return (
      <Modal 
        title={I18n.getText('apps.edit')}
